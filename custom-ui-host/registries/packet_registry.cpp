...
PacketRegistry::PacketRegistry(unsigned short range_min, unsigned short range_max)
    : range_min_(range_min), range_max_(range_max) {}

HandlerId PacketRegistry::Register(unsigned short opcode, PacketHandlerFn fn, void* user) {
    if (opcode < range_min_ || opcode > range_max_)
        return 0;
    std::lock_guard<std::mutex> g(mu_);
    if (by_opcode_.count(opcode) != 0)
        return 0;
...
void PacketRegistry::Dispatch(unsigned short opcode, const unsigned char* payload, unsigned int payloadLen) {
    if (opcode < range_min_ || opcode > range_max_)
        return;
