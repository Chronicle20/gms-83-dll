...
const std::string& First(const ms::ini::Parsed& p, const char* key, const std::string& fallback) {
...
void LoadHostConfig() {
    ...
    ms::ini::Parsed p;
    if (!ms::ini::Parse(apath, p, [](const char* msg) { Log("custom-ui-host: ini warning: %s", msg); })) {
    ...
    g_config.inbound_op_min = ParseHex16(First(p, "host.inbound_opcode_min", empty), 0x2000);
    g_config.inbound_op_max = ParseHex16(First(p, "host.inbound_opcode_max", empty), 0x20FF);
    g_config.outbound_op_min = ParseHex16(First(p, "host.outbound_opcode_min", empty), 0x0F00);
    g_config.outbound_op_max = ParseHex16(First(p, "host.outbound_opcode_max", empty), 0x0FFF);

    Log("custom-ui-host: config inbound=[0x%04X..0x%04X] outbound=[0x%04X..0x%04X] verbose=%d", g_config.inbound_op_min,
        g_config.inbound_op_max, g_config.outbound_op_min, g_config.outbound_op_max, (int)g_config.verbose);
}
