...
| CFuncKeyMappedMan.h | 0x388 (904) | **NEEDS-CHANGE — v79 member branch removing the 0x40 (FINAL, World-B closed task-16)** (Cat A→B). v79 must drop BOTH `m_aQuickslotKeyMapped[8]` + `m_aQuickslotKeyMapped_Old[8]` (0x20 each); member gates `>=111` (dummy1) / `>=95` (m_nNormalAttackCode) FALSE → v79 = base members minus the quickslot pair. No 0x388 assert (a size-assert can't express a member shift). | **0x388 re-confirmed by 2 INDEPENDENT anchors (task-16 disasm):** (1) `?CreateInstance@?$TSingleton@VCFuncKeyMappedMan@@@@` @0x946AFB `push 388h`→`Alloc(0x388=904)`; (2) ctor `??0CFuncKeyMappedMan@@QAE@XZ` @0x569DE5 field-init extent: vtable off_A2EB38@0, `memcpy 0x1BD`@+4 (m_aFuncKeyMapped[89], 5B elems), `memcpy 0x1BD`@+0x1C1 (m_aFuncKeyMapped_Old[89], ends 0x37E), then dwords zeroed @**+0x380/+0x384** → ends 0x388. **Absent 0x40 PINNED:** in the v83 layout the two quickslot int arrays occupy +0x380..+0x3C0; in v79 the ctor writes m_nPetConsumeItemID/MPItemID directly at +0x380/+0x384 (the slot the quickslot arrays would hold) → the pair [+0x380,+0x3C0) is ABSENT (PetConsume shifts down by 0x40; v83 PetConsume@0x3C0/0x3C4). Consistent with Task-7 DEFAULT_QKM_INSTANCE_ADDR=0x0 (no quickslot region to seed). Header computes 0x3C8 (968) — clang-cl probe `assert_size(...,0x388)` earlier FAILED `968 == 904` (Task 3), confirming the 0x40 member shift. NOTE: v79 cmake map `C_FUNC_KEY_MAPPED_MAN_CREATE_INSTANCE=0x009F9E98` is STALE (real = 0x946AFB) — flag for Task 7. |
...
| CMob.h | **0x518 (1304)** | **NEEDS-CHANGE — v79 base ≠ v83 (0x548)** (Cat C `>=84` fields absent ✓; Cat D doom region **absent**) | `?CreateMob@@` @0x630BF0 `push 518h`→`Alloc(0x518)`. ctor @0x630C2C highest member write `mov [esi+514h],edi` = `m_bWaitingToBeSetTossed` (last member); NO doom-zero tail, NO trailing ZList init. v83 typed CMob (0x548) has `m_bDoomReserved`@0x528/SN@0x52C/`m_lpStatChangeReserved`@0x530 — **all absent in v79** (would start at 0x518=past end). 0x30 delta (task-15, fully attributed) = **0x10 in `MobStat`** (the `nWeakness_/rWeakness_/tWeakness_` group + nFs realignment — see MobStat row) **+ 0x20 doom tail**; 0x548−0x10−0x20=0x518 ✓. Size re-anchored task-15: `?CreateMob@@`@0x630BF0 `push 518h`. doom-fix `<84` write would corrupt v79 → re-gate + header v79 branch in Task 17. |
| CMapLoadable.h | **0x114 (276)** | **unchanged** (Cat C `m_lVisibleByQuest` absent; `>=95` m_bField/m_lpLayerLetterBox/m_bPlayHoldedBGM/m_tPlayHoldedBGM absent) (task-15) | `?SetObjectState@CMapLoadable@@` @0x612aa4: `add ecx, 0A0h` → `m_mNamedObj` ZMap @0xA0 directly after `m_lpRefInfo`@0x8C; `m_lVisibleByQuest` (ZList) absent (else `m_mNamedObj` would be @0xB4). Tail anchor (task-15): `?OnSetBackEffect@CMapLoadable@@`@0x6145d2 `lea ecx,[esi+0B8h]` = `m_mTagedObj` ZMap @0xB8 (=0xA0+0x18). **Full size triangulated:** v95 typed 0x148 − m_bField(4) − m_lVisibleByQuest(0x14) − m_lpLayerLetterBox(0x14) − 2 BGM ints(8) = 0x114; cross-check v84=0x128 (task-006), whose ONLY layout diff vs v79 is the single m_lVisibleByQuest ZList → 0x128−0x14=0x114. No standalone v79 Alloc immediate (abstract base, inlined into derived ctors). |
...
| SecondaryStat.h | **0xB88 (2952)** | **unchanged** (base == v83; all `>=84`/`>=87`/`==87`/`>=95` gated tears ABSENT) (task-15) | **0xB88 independently confirmed** via the SecondaryStat ctor **sub_6F6D0C**: @0x6f6d2a `lea eax,[esi+0B50h]` + @0x6f6d30 `push 8` (elem size) + @0x6f6d28 `push 7` (count) → `eh vector constructor iterator` builds `aTemporaryStat[7]` @this+**0xB50**, 8-byte elems → ends 0xB88. Second witness: eh-vector **dtor** loc_9F41F0 `add eax,0B50h; push 8; push 7`. Anchor is DISTINCT from Task-13's embed-delta (m_secondaryStat@0x212C→m_forcedStat@0x2CB4=0xB88) and agrees. NOTE: `?Reset@SecondaryStat@@`@0x6fa9bd and `?DecodeForRemote@`@0x701539 iterate the array via `add esi,0B54h; [esi+8*i]` (= ZRef pointer at element+4) — looks like 0xB8C but the ctor/dtor base 0xB50 settles it at 0xB88 (no discrepancy). Array landing exactly at the v83 base offset 0xB50 proves the `>=84` Flying/Frozen, `>=87` DojangShield/AssistCharge/Enrage, and `>=95` tail tears are all ABSENT (each would push the array higher); v79 takes the `#else` 12-byte int branch like v83. |
| PartyData.h | **0x12A (298) — PACKED** | **unchanged gate** (`>=95` adwFieldID[6]+aTownPortal.m_nSKillID+aPQReward/aPQRewardType/dwPQRewardMobTemplateID/bPQReward all ABSENT ✓). ⚠ PACKING NOTE: header (no `#pragma pack`) computes 0x12C — 2B overshoot, Task-17 flag. (task-16) | `?Decode@PARTYDATA@@` @0x4d8693 does a FLAT `DecodeBuffer(this, 0x12A)` → serialized size 0x12A. In-memory confirmed PACKED via `?GetPartyTownPortal@CWvsContext@@` @0x988c43: adwCharacterID @CWvsContext+0x2D80, aTownPortal[0] @+0x2E4A → aTownPortal at PARTYDATA+**0xCA** (= sizeof PARTYMEMBER, NO 2B pad after asCharacterName[6][13]; 0x2E4A is 2-mod-4 → ints misaligned = packed). TOWNPORTAL = 16B (`movsd`×4, no m_nSKillID → `>=95` absent ✓). 0xCA + 6·0x10 = 0x12A == Decode size. `?GetPartyMemberNumber@` @0x988bbc iterates adwCharacterID[6] @+0x2D80. |
| PartyMember.h | **0xCA (202) — PACKED** | **unchanged gate** (`<95` adwFieldID[6] PRESENT for v79 ✓) | sizeof = aTownPortal offset within PARTYDATA = **0xCA** (above). Layout (packed, all from the flat decode + 0xCA landing): adwCharacterID[6]@0, asCharacterName[6][13]@0x18 (ends 0x66), anJob[6]@0x66, anLevel[6]@0x7E, anChannelID[6]@0x96, dwPartyBossCharacterID@0xAE, **adwFieldID[6]@0xB2** (ends 0xCA). Including adwFieldID pushes aTownPortal from 0xB2→0xCA (its 0x18 IS present) → `<95` branch confirmed. v95+ (field absent) would be 0xB2. ⚠ PACKING NOTE: unpacked header = 0xCC. (task-16) |
| GuildData.h | **0x2A (42) — PACKED** | **unchanged gate** (`>=95` nLevel + mSkillRecord ZMap + aSkillRecordOnlyID ZArray all ABSENT ✓). ⚠ PACKING NOTE: unpacked header computes 0x2C — Task-17 flag. (task-16) | GUILDDATA @CWvsContext+0x2EE8 (`?GetGuildMemberGrade@CWvsContext@@` @0x51be82). Size 0x2A by **2 independent anchors**: (1) Clear `sub_94E8E3` field-init extent — writes nGuildID@0, sGuildName ZXString@4, asGradeName ZArray@8, adwCharacterID ZArray@0xC, aMemberData ZArray@0x10, nMaxMemberNum@0x14, nMarkBg u16@0x18, nMarkBgColor u8@0x1A, nMark u16@**0x1B** (odd → PACKED), nMarkColor u8@0x1D, sNotice ZXString@0x1E, nPoint@0x22, **nAllianceID@0x26** (last write, ends 0x2A); NO nLevel/ZMap/ZArray init past 0x26 → `>=95` block absent. (2) Guild-list array stride = **0x2A** (`?OnGuildResult@` @0x980ab9 `add [..],2Ah` iterating GUILDDATA[] @+0x2F44, comparing element nGuildID@0). `?GetCount@GUILDDATA@@` @0x51beae reads aMemberData@0xC; GUILDMEMBER stride 0x25 (@0x51be9e `imul eax,25h`). ⚠ PACKING NOTE: unpacked header = 0x2C. (task-16) |
| MobStat.h | **v79 0x1F8 (504) / v83 0x208 (520)** | **NEEDS-CHANGE — v79 lacks the `nWeakness_/rWeakness_/tWeakness_` group (currently UNGATED, lines 125-127)** (`>=95` nTimeBomb_/nMagicCrash_/bCannotEvade blocks correctly ABSENT) (task-15) | The 0x10 shrink = Weakness group (0xC) + 4B realignment before `long double nFs`. v79 CMob ctor @0x630d1e `lea eax,[ebx+1E0h]` (ebx=&m_stat@0x1A0) builds `lBurnedInfo` (last member) @MobStat-internal **0x1E0**; v83 ctor @0x6622cb `lea eax,[edi+1F0h]` @**0x1F0** → sizeof 0x1F8 vs 0x208. `m_nTeamForMCarnival` (0FFh) v79@0x3A0 / v83@0x3B0. Reset corroboration: v83 `?Reset@MobStat@@`@0x78c187 has an `MS_Weakness` conditional group @0x1D0/0x1D4/0x1D8; v79 `?Reset@`@0x70493d has **none** (ends at BodyPressure 0x1C4-0x1CC). All stat offsets through tBodyPressure_@0x1CC are byte-identical v79↔v83. Refines the Task-12 "~0x10 in MobStat" into the specific field. Task 17 must gate the Weakness group to exclude v79. |
