...
| CWnd.h | **0x64 (100)** | **NEEDS-CHANGE — v79 lacks `m_ptCursorRel` (CWnd 0x64 vs v83 0x6C, −8)** (>=95 UIOrigin enum + m_origin ABSENT ✓; >=87 SECPOINT ABSENT ✓ — but the `<87` else-branch `int m_ptCursorRel_x/_y` (8 B) OVERSHOOTS v79) (task-14) | CONTROL BASE CLASS. v79 m_lpChildren ZList@**0x48** (vs v83 0x50), m_pFocusChild@**0x5C** (vs v83 0x64), m_pBackgrnd@**0x60** (vs v83 0x68) — all −8. Anchors: `?Destroy@CWnd@@` @0x92f2f1 (`lea eax,[esi+48h]`→ZList::AddTail on m_lpChildren; `mov [esi+5Ch],ebx`→clear m_pFocusChild); base-init helper sub_92D5ED writes m_lpChildren-vtable@0x48 + through 0x60; a direct CWnd-subclass sub_4632BB places its first own member @**0x64** (= sizeof CWnd). v83 has m_ptCursorRelX@0x48/Y@0x4C immediately before m_lpChildren@0x50 (typed struct); v79 has m_lpChildren there → the 8-byte m_ptCursorRel pair is ABSENT in v79. ⚠ CONCERN: the named `??0CWnd@@QAE@XZ` @0x4e168d zeroes [esi+64h]/[68h]/[70h] (would imply 0x74); this is inconsistent with the 4 runtime anchors + the CUIWnd/CFadeWnd evidence — flag for Task-17, but the −8/0x64 size is firmly multiply-confirmed. |
| CUIWnd.h | **0x5A8 (1448)** | **NEEDS-CHANGE — inherits the CWnd −8 shift** (>=95 5-field block m_nSmallScreenX/Y,m_nLargeScreenX/Y,m_bIsLargeMode ABSENT ✓) (task-14) | CONTROL BASE CLASS. `CUIWnd : CWnd`(0x64). m_pBtClose(ZRef, 8B)@0x64, m_uiToolTip CUIToolTip(0x514)@**0x6C**..0x580, m_nUIType@0x580, **m_nBtCloseType@0x584** (`?OnCreate@CUIWnd@@` @0x8862c7 switches on [esi+584h] 1..4 = close-btn corner), … **m_sBackgrndUOL(ZXString)@0x5A4** last (`?ReloadBackgrnd@CUIWnd@@` @0x8866dc `lea ecx,[esi+5A4h]`→ZXString op + cond@[esi+590h]). Every offset is −8 vs v83 typed CUIWnd (m_nBtCloseType@0x58C, m_sBackgrndUOL@0x5AC, size 0x5B0). >=95 block absent (else m_sBackgrndUOL@0x5B8). sizeof v79 = 0x5A8. The −8 originates entirely in the CWnd base (m_ptCursorRel). |
